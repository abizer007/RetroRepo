...
const LEGEND_INSET_RIGHT = 48
const LEGEND_INSET_TOP = 28
const LEGEND_ROW_GAP = 14
...
export function generateTimelineSvg(repos: TimelineRepo[], username: string): string {
  ...
  const legendAreaY = legendZoneTop + LEGEND_INSET_TOP
  const legendStartX = width - padding - LEGEND_INSET_RIGHT
  const legendItemSpacing = 46
  ...
  for (let i = languagesPresent.length - 1; i >= 0; i--) {
    ...
    legendParts.push(`
      ...
      <text x="${legendX + 10}" y="${legendAreaY}" text-anchor="start" font-size="9" fill="var(--color-text-light)">${escapeXml(displayLang)}</text>
    `)
    legendX -= legendItemSpacing
  }
  const legendSvg = legendParts.length > 0 ? `
      <text x="${legendStartX}" y="${legendAreaY - LEGEND_ROW_GAP}" text-anchor="end" font-size="9" fill="var(--color-text-light)" opacity="0.85">Lang</text>
      ${legendParts.join("")}
  ` : ""
  ...
  return `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="${padding}" y="${height - 26}" text-anchor="start" font-size="10" fill="var(--color-text-light)" opacity="0.7">
